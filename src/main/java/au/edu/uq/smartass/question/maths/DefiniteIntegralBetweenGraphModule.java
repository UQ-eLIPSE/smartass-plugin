...
public class DefiniteIntegralBetweenGraphModule implements QuestionModule {
	final int POLYNOM = 0;
	final int SIN = 1;
	final int COS = 2;
	...
	public DefiniteIntegralBetweenGraphModule() {
		super();
		...
		case POLYNOM: {
			...
			consts[0] = RandomChoice.randInt(0,1)==0?
					RandomChoice.randInt(-MAX_CONST*MAX_CONST*2, -MAX_CONST*MAX_CONST):
						RandomChoice.randInt(MAX_CONST*MAX_CONST, MAX_CONST*MAX_CONST*2); 
			consts[1] = RandomChoice.randInt(-MAX_CONST, MAX_CONST);
			consts[2] = RandomChoice.randInt(-MAX_CONST/2, MAX_CONST/2);
		if(consts[2]==0)
				consts[3] = RandomChoice.randInt(1, MAX_CONST/2);
			else
				consts[3] = RandomChoice.randInt(-MAX_CONST/2, MAX_CONST/2);

			
			int i1 = RandomChoice.randInt(-MAX_X, MAX_X-1);
			while(Math.abs(polyFunc(consts, MAX_X)/(polyFunc(consts, i1)+0.0001))>15 && i1<MAX_X-1)
				i1++;
			//if(polyFunc(consts, MAX_X)/(polyFunc(consts, 0)+0.0001)<=10)
//				i1 = RandomChoice.randInt(-MAX_X, MAX_X-1);
			//else
//				i1 = RandomChoice.randInt(MAX_X/2, MAX_X-1);
		...
		case SIN: {
			...
			int[] muls = {-1, -3, -1, -1, 0, 1, 1, 3, 1};
			int[] divs = {1, 4, 2, 4, 1, 4, 2, 4, 1};

			int i1 = RandomChoice.randInt(0, divs.length-2);
			int i2 = RandomChoice.randInt(i1+1, divs.length-(i1==0?2:1));
		...
		case COS:
			...
			int[] muls = {-1, -1, 0, 1, 1, 3, 1, 3};
			int[] divs = {2, 4, 1, 4, 2, 4, 1, 2};
			int i1 = RandomChoice.randInt(0, divs.length-2);
			int i2 = RandomChoice.randInt(i1+1, divs.length-(i1==0?2:1));
			nx1 = Math.PI*muls[i1]/divs[i1];
			nx2 = Math.PI*muls[i2]/divs[i2];
